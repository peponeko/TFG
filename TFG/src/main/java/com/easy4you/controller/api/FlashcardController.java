...
public class FlashcardController {
  ...
  public ResponseEntity<List<FlashcardResponseDTO>> listarPorDocumento(@PathVariable Long documentoId) {
    ...
    return ResponseEntity.ok(flashcardService.listarPorDocumento(usuarioId, documentoId));
  }

  @GetMapping("/tema/{temaId}")
  public ResponseEntity<List<FlashcardResponseDTO>> listarPorTema(@PathVariable Long temaId) {
    Long usuarioId = authenticatedUserService.requireUsuarioActual().getId();
    return ResponseEntity.ok(flashcardService.listarPorTema(usuarioId, temaId));
  }
  ...
  public ResponseEntity<FlashcardResponseDTO> actualizar(
    ...
    return ResponseEntity.ok(FlashcardMapper.toResponse(updated));
  }
